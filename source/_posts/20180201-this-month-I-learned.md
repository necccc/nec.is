...
- image data as binary
    - tainted canvas
    - xhr load array buffer
    - stream binary data with fetch
- typed arrays
    - subarray vs splice
    - concatenating TypedArrays
    - set data in TypedArrays
- Web Crypto API
- quick https server for your localhost using docker & nginx
- embedding google fonts
- test your code against all sort of HTTP status code responses out there
...
If you’ve managed to serve these assets using CORS, there is a more elegant way to get the binary data of images. Simply load them using XHR, but set the `responseType` to “array buffer”.
...
### Streaming binary data using Fetch
...
## HTTP Response Statuscode test service
